...
export const LoginPage = () => {
  const { login, isLoading, testLogin } = useAuth();
  ...
  return (
    <div className="h-full w-full flex items-center justify-center bg-gradient-to-br from-slate-900 to-slate-800 overflow-auto" style={{ paddingTop: 'env(safe-area-inset-top)', paddingBottom: 'env(safe-area-inset-bottom)' }}>
      <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-8 max-w-md w-full mx-4 shadow-2xl">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-white mb-2">MandalaPlan</h1>
          <p className="text-slate-300">목표 달성을 위한 만다라트 플래너</p>
        </div>
        ...
        <p className="text-slate-400 text-sm text-center mt-6">
          로그인하면 <Link href="/privacy" className="underline hover:text-white transition-colors">서비스 이용약관 및 개인정보처리방침</Link>에 동의하게 됩니다.
        </p>
      </div>
    </div>
  );
